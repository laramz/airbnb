...
When submitting it is important to note: for a certificate, each person must submit the project individually. However, this can be identical within your group. You can get more information at our first Coding Meetup on \textbf{November xx, 2020}.

This Airbnb case study and the associated project guide was developed and written entirely from scratch by TechAcademy's Data Science team. Lukas Jürgensmeier and Lara Zaremba developed the project in \texttt{R}, while Felix Schneider and Manuel Mair am Tinkhof developed it in \texttt{Python}.
...
Of course, we will also go into more detail about the project and the tools that go with it. We will discuss everything you need to know during the first Coding Meetup, which will take place on \textbf{November xx, 2020}. After that, the work on the project will officially begin.
...
Last semester we dealt with the COVID-19 outbreak, but to take your mind off things, we thought it would be a good idea to focus this semester on a different topic - the business model of Airbnb. More precisely, we are analyzing part of a very detailed data set of all Airbnb offers in Berlin. The data was scraped in November 2019, which means it was taken directly from the Airbnb website itself. You will find all kinds of information in the data set - useful and useless ones.
...
As a first step you will get to know the data set. This means you will describe the data and answer questions like ``What variables are contained in the data set? \ldots and how are they related?''. For this you can often use graphical tools like box plots or histograms.
...
Now you have access to the tech academy drive. There, you can navigate to the ``data'' folder with all the data sets you need for this project.
Alternatively, you can download the data folder from the tech academy drive and upload it to google colab.
...
Before we can start with that, we first have to clean the data such that the functions can process the data set properly. You will have to do that quite often in the field of Data Science, very often the data comes in an unusable format. In our case, for example, the values in the price column are stored as type \texttt{string} -- this becomes an issue when performing math operations.
...
To make the next step easier, we still have to convert the logical variable \texttt{available} from a \texttt{factor} to a \texttt{boolean} data type. Use the \texttt{ifelse()} function for this
...
Now, understand the next step, remember that you converted the values in the ``available'' column to be of type bool before. In Python, the boolean value True is equal to 1, while False is equal to 0. Thus, (True + True + False) is equal to 2 (try it out: \texttt{print(True\ +\ True\ +\ False)}), and thus, you could now sum the entries in the available column to get the number of available apartments.
...
After looking at the availability over time using the \texttt{calendar} data set in the first step, we would now like to find out a little more about the price structures of the available apartments. For this we need the \texttt{listings} data set that we can load into the workspace as usual. A closer look reveals that the \texttt{price} and \texttt{cleaning\_fee} columns contain characters and once again need to be cleaned.

Therefore, as in the first task, the dollar sign as well as the comma must be deleted from these columns. Since you use the same procedure for the \texttt{price} and \texttt{cleaning\_fee} columns, a for-loop is recommended here. Implement this loop in a self-written function \texttt{clean\_price()}, so that you save a lot of lines of code, especially in the prediction part.
...
You can easily filter in the plot specifications with \texttt{xlim} or \texttt{ylim}.
...
For plotting: The Matplotlib documentation is a great source of inspiration with many examples incl.~code that generated them.
...
\hypertarget{merging-of-listings-and-review-data-set}{%
\subsection{Merging of listings and review data set}\label{merging-of-listings-and-review-data-set}}
...
listings\_reviews \textless- merge(dataset1, dataset2, by = \ldots)
...
We can then place actual objects on the map. Plot the 200 top rated listings on the map. If you have not yet solved task 3, simply select 200 listings according to other criteria or randomly to solve the task. It could look like this:
...
You can then define a vector, which you can call ``berlin\_borders'', for example. It is used to define the values for the edges of the map. You can add a small safety margin to the respective minima or maxima of the coordinates. Play around with the factors later to find a good section of the map.
...
We are going to use yet another library, this one is called ``ipyleaflet''. For this task you will have to work with the documentation for this library.
...
In addition to the GPS coordinates, there is a lot of information about each listing in our data. This time plot the apartments in different colors.
...
To do so you can recycle some parts of the code from before, but this time you work with a nested for-loop (loop inside of a loop).
...
\hypertarget{price-prediction--the-application-of-statistical-methods}{%
\section{Price Prediction- The application of statistical methods}\label{price-prediction--the-application-of-statistical-methods}}
...
\begin{itemize}
\item
  \emph{train.csv} (60 \%): You use this training data set to train your model. The model can learn the relationships between the variables by being estimated based on the training data set that contains both, the variables needed to predict the prices and also the prices themselves.
\item
...
\hypertarget{examine-the-correlation-between-the-variables-test}{%
\subsection{Examine the Correlation Between the Variables (test)}\label{examine-the-correlation-between-the-variables-test}}
...
The GGally package with the \texttt{ggpairs\ ()} function provides a very practical plot for visualizing relationships between many variables.
...
Use its pairplot method and pass it the dataframe with the selected columns to visualize distributions and correlations.
...
In \texttt{R} you can implement a simple linear regression with the function \texttt{lm()}. You then get the model estimation with the \texttt{summary()} function.
...
Does your independent variable have a statistically significant impact on apartment price? Probably yes, because we selected the variable most correlated with price. However, if we stick to this very simplified model, we are making a typical mistake: the so-called Omitted Variable Bias (OVB). To put it simply, we neglect (in statistics jargon: ``do not control for'') variables that have a significant influence on the dependent variable. One could suspect that other influencing factors also play a large role in price setting. If we do not include them, the estimate of the effect of \texttt{bedrooms}is distorted and thus hardly useful. In our case this is not a big problem for the time being, since we are not interested in causal effects, but rather in the best possible prediction. Your statistics professor would almost certainly object to such a model. Nonetheless, with just a single explanatory variable, this model will not necessarily predict the price well.

One possible solution is to simply include the neglected variables in the model - how practical that these are already included in the data set. So let's set up a somewhat more extensive model that includes one more variable:
...
\texttt{\#\ Function\ that\ returns\ Root\ Mean\ Squared\ Error\ while\ ignoring\ NAs}
...
After training, import the data from the test.csv file, define both variables \texttt{X\_test}and \texttt{y\_test}, and create a vector with price predictions applying \texttt{.predict(X\_test)} on your model. Store your prediction in the variable \texttt{predicted\_price}.
...
included in our data set. However, you can write a simple function that uses the two coordinate variables to calculate the distance to the center of Berlin and appends this to the data set as a new variable.
...
Did you find your best model? Then predict prices with your winning model as above - but this time on the validation data set val.
...
\textbf{Part A: Exploratory Data Analysis (Beginner + Advanced)}
...
** Part B: price prediction using statistical methods (advanced and motivated beginners)

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Visualize feature correlations in heatmap
\item
  Regression
